...
DROP FUNCTION IF EXISTS `moocdb`.get_average_watching_speed_per_user_per_week;
...
CREATE FUNCTION get_average_watching_speed_per_user_per_week (USER_ID VARCHAR(63), FEATURE_WEEK INT(2), USER_START_TIME TIMESTAMP)
    RETURNS FLOAT
    NOT DETERMINISTIC
BEGIN
  ...
  cum_loop: LOOP
    ...
    IF count_flag THEN
        ...
        ELSEIF event_type = 'speed_change_video' THEN
            ...
            SET duration = TIMESTAMPDIFF(SECOND, start_time_stamp, end_time_stamp) * speed;
            SET cum_time = cum_time + duration;
            SET cum_product = cum_product + duration * speed;
...
SELECT 791,
	...
	get_average_watching_speed_per_user_per_week(user_long_feature_base.user_id, user_long_feature_base.feature_week, (
