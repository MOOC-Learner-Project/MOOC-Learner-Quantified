DROP FUNCTION IF EXISTS `moocdb`.get_number_of_session_per_user_per_week;

CREATE FUNCTION get_number_of_session_per_user_per_week (USER_ID VARCHAR(63), FEATURE_WEEK INT(2), USER_START_TIME TIMESTAMP)
    RETURNS FLOAT
    NOT DETERMINISTIC
BEGIN
...
SELECT 731,
	...
	(
	    ...
	    FROM `moocdb`.user_long_feature
	    WHERE user_long_feature.feature_id = 721
	        AND user_long_feature.user_id = user_long_feature_base.user_id
	        AND user_long_feature.feature_week = user_long_feature_base.feature_week
	)
	/
	NULLIF(get_number_of_session_per_user_per_week(user_long_feature_base.user_id, user_long_feature_base.feature_week, (
